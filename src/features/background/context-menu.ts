...
interface MenuProperties {
  title: string;
  contexts: [Browser.contextMenus.ContextType, ...Browser.contextMenus.ContextType[]];
}
...
async function ensureContextMenu(id: string, properties: MenuProperties): Promise<void> {
  try {
    await browser.contextMenus.update(id, properties);
  } catch {
    try {
      await browser.contextMenus.create({ id, ...properties });
    } catch {
...
async function routeContextMenuClick(info: Browser.contextMenus.OnClickData, tab?: Browser.tabs.Tab): Promise<void> {
  ...
  await routeUrl(target.url, target.pageUrl, `context_menu_${target.source}`, target.filename, target.captureType);
}
