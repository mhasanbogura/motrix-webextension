  ...
  return (
    <Section title={t('options.download')} icon={Download} compact={compact}>
      <div className={cn('grid gap-(--options-gap)', compact ? 'grid-cols-2 max-[860px]:grid-cols-1' : 'grid-cols-1')}>
        ...
        <SettingSwitch
          compact={compact}
          label={t('options.promptBeforeDownload')}
          hint={t('options.promptBeforeDownloadHint')}
          checked={snapshot.settings.promptBeforeDownload}
          onCheckedChange={(promptBeforeDownload) => updateSettings({ promptBeforeDownload })}
        />
      </div>
